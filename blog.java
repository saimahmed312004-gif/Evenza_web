    ...
    function filterPosts(btn, cat) {
      ...
      document.querySelectorAll('.post-c').forEach(p => {
    ...
    document.getElementById('searchInput').addEventListener('input', function () {
      ...
      document.querySelectorAll('.post-c').forEach(p => {
        const txt = p.querySelector('.post-title').textContent.toLowerCase();
        p.style.display = txt.includes(q) ? 'flex' : 'none';
      });
    });
    ...
    //     if (isTo) return; e.preventDefault();
    //     let d = e.deltaY; if (e.deltaMode === 1) d *= 32; if (e.deltaMode === 2) d *= window.innerHeight;
    //     d = clamp(d, -MAXD, MAXD) * WMULT; tgt = clamp(tgt + d, 0, max()); go();
    //   }, { passive: false });
    ...
    //     function anim(now) {
    //       const p = Math.min((now - st) / dur, 1), ea = easeOut(p);
    //       cur = sy + dist * ea; tgt = cur; window.scrollTo(0, cur);
    //       if (p < 1) { raf = requestAnimationFrame(anim); } else { cur = dest; tgt = dest; window.scrollTo(0, dest); raf = null; isTo = false; }
    //     }
