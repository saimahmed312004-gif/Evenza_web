        ...
        // Inquiry type
        function selectInq(btn) {
            document.querySelectorAll('.inq-btn').forEach(b => b.classList.remove('on'));
            btn.classList.add('on');
        }

        // Budget selector
        function selectBudget(btn) {
            document.querySelectorAll('.budget-btn').forEach(b => b.classList.remove('on'));
            btn.classList.add('on');
        }
        ...
        if (fileInput) {
            fileInput.addEventListener('change', () => {
                if (fileInput.files[0]) {
                    ...
                    fileDrop.style.borderColor = 'rgba(0,245,212,.4)';
                    fileDrop.style.background = 'rgba(0,245,212,.05)';
                }
            });
        }
