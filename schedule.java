    ...
    // Day switcher
    function switchDay(btn, id) {
      document.querySelectorAll('.day-tab').forEach(t => t.classList.remove('on'));
      document.querySelectorAll('.day-panel').forEach(p => p.classList.remove('on'));
      btn.classList.add('on');
      document.getElementById(id).classList.add('on');
    }

    // Session filter
    function filterSessions(btn, type) {
      const panel = btn.closest('.day-panel');
      panel.querySelectorAll('.filter-btn').forEach(b => b.classList.remove('on'));
      btn.classList.add('on');
      panel.querySelectorAll('.sr').forEach(sr => {
        sr.style.display = (type === 'all' || sr.dataset.type === type) ? 'grid' : 'none';
      });
    }
    ...
    //   function loop() {
    //     const d = tgt - cur;
    //     if (Math.abs(d) < 0.5 && Math.abs(vel) < 0.5) { cur = tgt; window.scrollTo(0, cur); raf = null; return; }
    //     cur += d * EASE + vel; vel *= FRIC; cur = clamp(cur, 0, max()); window.scrollTo(0, cur); raf = requestAnimationFrame(loop);
    //   }
    ...
    //     if (isTo) return;
    //     const map = { ArrowDown: 100, ArrowUp: -100, PageDown: window.innerHeight * .85, PageUp: -(window.innerHeight * .85), ' ': window.innerHeight * .85 };
    //     if (map[e.key] !== undefined) { e.preventDefault(); tgt = clamp(tgt + map[e.key], 0, max()); go(); }
    //     if (e.key === 'Home') { tgt = 0; go(); } if (e.key === 'End') { tgt = max(); go(); }
    //   });
    //   function scrollTo(dest, dur) {
    //     dur = dur || 900; isTo = true; const sy = cur, dist = dest - sy, st = performance.now();
    //     if (raf) { cancelAnimationFrame(raf); raf = null; }
    //     function anim(now) { const p = Math.min((now - st) / dur, 1), ea = easeOut(p); cur = sy + dist * ea; tgt = cur; window.scrollTo(0, cur); if (p < 1) { raf = requestAnimationFrame(anim); } else { cur = dest; tgt = dest; window.scrollTo(0, dest); raf = null; isTo = false; } }
    //     raf = requestAnimationFrame(anim);
    //   }
    //   document.addEventListener('click', e => {
    //     const a = e.target.closest('a[href^="#"]'); if (!a) return;
    //     const id = a.getAttribute('href').slice(1); if (!id) { e.preventDefault(); scrollTo(0); return; }
    //     const el = document.getElementById(id); if (!el) return;
    //     e.preventDefault(); scrollTo(clamp(el.getBoundingClientRect().top + window.scrollY - 80, 0, max()));
    //   });
