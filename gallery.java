    ...
    function filterGal(btn, cat) {
      ...
      document.querySelectorAll('.gal-item').forEach(i => {
    ...
    //   window.addEventListener('wheel', e => { if (isTo || document.getElementById('lightbox').classList.contains('on')) return; e.preventDefault(); let d = e.deltaY; if (e.deltaMode === 1) d *= 32; if (e.deltaMode === 2) d *= window.innerHeight; d = clamp(d, -MAXD, MAXD) * WMULT; tgt = clamp(tgt + d, 0, max()); go(); }, { passive: false });
    ...
    //   window.addEventListener('keydown', e => { if (isTo) return; const map = { ArrowDown: 100, ArrowUp: -100, PageDown: window.innerHeight * .85, PageUp: -(window.innerHeight * .85), ' ': window.innerHeight * .85 }; if (map[e.key] !== undefined && !document.getElementById('lightbox').classList.contains('on')) { e.preventDefault(); tgt = clamp(tgt + map[e.key], 0, max()); go(); } if (e.key === 'Home') { tgt = 0; go(); } if (e.key === 'End') { tgt = max(); go(); } });
