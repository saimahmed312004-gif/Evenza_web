    ...
    //     cur += d * EASE + vel; vel *= FRIC;
    //     cur = clamp(cur, 0, max()); window.scrollTo(0, cur); raf = requestAnimationFrame(loop);
    //   }
    //   function go() { if (!raf) raf = requestAnimationFrame(loop); }
    //   window.addEventListener('wheel', e => {
    //     if (isTo) return; e.preventDefault();
    //     let d = e.deltaY;
    //     if (e.deltaMode === 1) d *= 32; if (e.deltaMode === 2) d *= window.innerHeight;
    ...
    //     dur = dur || 900; isTo = true;
    //     const sy = cur, dist = dest - sy, st = performance.now();
    ...
    //       else { cur = dest; tgt = dest; window.scrollTo(0, dest); raf = null; isTo = false; }
    //     }
    ...
    //     if (!el) return;
    //     e.preventDefault();
    //     scrollTo(clamp(el.getBoundingClientRect().top + window.scrollY - 80, 0, max()));
    //   });
